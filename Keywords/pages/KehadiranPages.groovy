...
public class KehadiranPages {
	...
	private TestObject btnExport1
	private TestObject btnExport2
	private TestObject btnBatal

	def KehadiranPages(String StartMonth, String StartYear, String StartDate, String EndMonth, String EndYear, String EndDate) {
		//Search Name
		...
		//Export
		btnExport1 = ObjectGlobal.getTestObject("//button[contains(@class, 'MuiButton-containedPrimary') and contains(text(), 'Export') and contains(@class, 'css-112fgj')]")
		btnExport2 = ObjectGlobal.getTestObject("//button[contains(@class, 'MuiButton-containedPrimary') and contains(text(), 'Export') and contains(@class, 'css-4075ia')]")
		btnBatal = ObjectGlobal.getTestObject("//button[contains(@class, 'MuiButton-textSecondary') and contains(text(), 'Batal')]")
	}

	def kehadiran(String search, String Departmen) {
		//Search Name
		...
		//		WebUI.takeScreenshot("screenshots/search_results.png")
		//Filter Departemen
		...
		for (int i = 0; i < 1; i++) {
			if (WebUI.verifyElementClickable(btnLastPages, FailureHandling.OPTIONAL)) {
				...
				//				WebUI.click(btnReset) // Click reset button if Last Page is not active
				//				WebUI.delay(2) // Wait for page reset
		...
		for (int i = 0; i < 1; i++) {
			if (WebUI.verifyElementClickable(btnFirstPage, FailureHandling.OPTIONAL)) {
				...
				//				WebUI.click(btnReset) // Click reset button if First Page is not active
				//				WebUI.delay(2) // Wait for page reset
		...
		// Export
		WebUI.click(btnExport1)
		WebUI.click(btnExport2)
		// Take Screenshot after clicking the second Export button
		WebUI.takeScreenshot("screenshots/after_export2.png")
